...
void teste(){
	...
	bufA = clCreateBuffer(context, CL_MEM_READ_ONLY, 40, NULL, &err);
	cl_error(err);
	bufB = clCreateBuffer(context, CL_MEM_READ_ONLY, 40, NULL, &err);
	cl_error(err);
	bufC = clCreateBuffer(context, CL_MEM_READ_WRITE, 40, NULL, &err);
	cl_error(err);
